...
public interface ProdutoRepositorio extends JpaRepository<Produto, Long> {
	...
	// essa primeira consulta não tras as embalagens e categorias para a memoria, o
	// que acaba resultando em várias consultas ao banco
	@Query("SELECT DISTINCT objeto FROM Produto objeto " + "INNER JOIN objeto.embalagens embals "
	...
	// join fetch faz com que os relacionamentos sejam carregados junto com a
	// entidade.
	// join desabilita o carregamento LAZY, assim não precisa ir tocar várias vezes
	// no database
	@Query("SELECT objeto " + "FROM Produto objeto " + "JOIN FETCH objeto.embalagens " + "JOIN FETCH objeto.categorias "
